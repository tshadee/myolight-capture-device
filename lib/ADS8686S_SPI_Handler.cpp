...
ADS8686S_SPI_Handler::ADS8686S_SPI_Handler(int SDI, int SDO, int CS, int SCLK, int RST, int CONVST, int BUSY, SPIClass &spiInstance) : SDI(SDI), SDO(SDO), CS(CS), SCLK(SCLK), RST(RST), CONVST(CONVST), BUSY(BUSY), vspi(spiInstance)
,configArr{CONFIG_DEFAULT_CFG, RANGE_A1_DEFAULT_CFG, RANGE_A2_DEFAULT_CFG, RANGE_B1_DEFAULT_CFG, RANGE_B2_DEFAULT_CFG, LPF_DEFAULT_CFG, SEQ_STACK_0_DEFAULT_CFG, SEQ_STACK_1_DEFAULT_CFG, SEQ_STACK_2_DEFAULT_CFG, SEQ_STACK_3_DEFAULT_CFG}
{
    for(int i = 0; i < rxBufferDepth; i++)    { rxBuffer[i] = 0x00; };
    for(int i = 0; i < txBufferDepth; i++)    { txBuffer[i] = 0x00; };
};
