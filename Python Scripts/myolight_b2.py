...
def analyse_data():
    ...
    packet_numbers = data[:,-1]

    for i in range(len(channels)):
        channels[i] = channels[i].astype(float)
        channels[i] *= (2.50 / 32767.0) #normalise and scale to FSR

    #TODO: this also has to be according to user input
    sample_rate = 500 #hz
    ...
    fig, axes = plt.subplots(4, 8, figsize=(20, 10))  # 4 rows, 8 columns
    fig.suptitle("FFT of All 32 Channels", fontsize=12)
    
    for i, fft_result in enumerate(fft_results):
        row = (31-i) // 8  # Determine the row index
        col = i % 8   # Determine the column index
        ax = axes[row, col]
